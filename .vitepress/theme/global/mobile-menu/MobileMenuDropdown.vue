...
function toggle() {
  ...
  vh.value =
    window.innerHeight +
    Math.min(
      window.scrollY - parseInt(getComputedStyle(document.documentElement).getPropertyValue("--vp-nav-height")),
      0
    );
}
...
<template>
  <div class="MobileMenuDropdown" :style="{ '--vp-vh': vh + 'px' }" ref="main" data-allow-mismatch="style">
...
.MobileMenuDropdown button:hover {
  color: var(--vp-c-text-1);
  transition: color 0.25s;
}

.MobileMenuDropdown button.open {
  color: var(--vp-c-text-1);
}
...
@media (min-width: 960px) {
  .items {
    right: auto;
    left: calc(var(--vp-aside-width) + 32px);
    width: 320px;
  }
}
