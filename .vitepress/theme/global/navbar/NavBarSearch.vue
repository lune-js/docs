...
import NavBarAskAiButton from "./NavBarAskAiButton.vue";
import NavBarSearchButton from "./NavBarSearchButton.vue";

const VPLocalSearchBox = __VP_LOCAL_SEARCH__
  ? defineAsyncComponent(() => import("../search/LocalSearchBox.vue"))
  : () => null;

const VPAlgoliaSearchBox = __ALGOLIA__
  ? defineAsyncComponent(() => import("../search/AlgoliaSearchBox.vue"))
  : () => null;
...
<template>
  <div class="NavBarSearch">
    <template v-if="provider === 'algolia'">
      ...
      />
      <VPAlgoliaSearchBox v-if="loaded" :algolia-options :open-request @vue:beforeMount="actuallyLoaded = true" />
    </template>
