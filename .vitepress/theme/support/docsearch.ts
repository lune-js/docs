...
export function resolveMode(
  options: Pick<DefaultTheme.AlgoliaSearchOptions, "appId" | "apiKey" | "indexName" | "askAi" | "mode">
): ResolvedMode {
...
export function hasKeywordSearch(
  options: Pick<DefaultTheme.AlgoliaSearchOptions, "appId" | "apiKey" | "indexName">
): boolean {
  return Boolean(options.appId && options.apiKey && options.indexName);
}

export function hasAskAi(askAi: DefaultTheme.AlgoliaSearchOptions["askAi"]): boolean {
...
export function buildAskAiConfig(
  ...
  const askAiSearchParameters =
    !isAskAiString && askAiProp.searchParameters ? { ...askAiProp.searchParameters } : undefined;
  ...
  if (mergedAskAiSearchParameters && Object.values(mergedAskAiSearchParameters).some((v) => v != null)) {
...
export function buildSidePanelProps(
  ...
  const { sidePanel, ...askAiRest } = JSON.parse(JSON.stringify(askAi)) as DocSearchAskAi;
...
export function resolveOptionsForLanguage(
  ...
  const askAi = options.askAi ? buildAskAiConfig(options.askAi, options, lang) : undefined;
