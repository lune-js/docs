...
import { isActive, ensureStartingSlash } from "./utils";
...
export function getSidebar(
  ...
  // * NOTE: It is important the developers know the order must match their page navigation
  if (activeMatch && path.includes(activeMatch)) {
    return Object.keys(_sidebar).reduce<SidebarItem[]>((sidebar, currVal) => {
      const item = _sidebar[currVal];

      if (Array.isArray(item)) sidebar.push(...addBase(item));
      else sidebar.push(...addBase(item.items, item.base));
...
export function addBase(items: SidebarItem[], _base?: string): SidebarItem[] {
  return [...items].map((_item) => {
    const item = { ..._item };
    const base = item.base || _base;
    if (base && item.link) item.link = base + item.link.replace(/^\//, base.endsWith("/") ? "" : "/");
